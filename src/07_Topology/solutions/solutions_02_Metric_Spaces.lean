...
theorem solutions_02_Metric_Spaces_1 {u : ℕ → X} {a : X} :
  tendsto u at_top (𝓝 a) ↔ ∀ ε > 0, ∃ N, ∀ n ≥ N, dist (u n) a < ε :=
metric.tendsto_at_top

theorem solutions_02_Metric_Spaces_2 {X Y : Type*} [metric_space X] [metric_space Y] {f : X → Y} :
  continuous f ↔
...
theorem solutions_02_Metric_Spaces_3 {X Y : Type*} [metric_space X] [metric_space Y] {f : X → Y} (hf : continuous f) :
  continuous (λ p : X × X, dist (f p.1) (f p.2)) :=
by continuity


theorem solutions_02_Metric_Spaces_4 {X Y : Type*} [metric_space X] [metric_space Y] {f : X → Y} (hf : continuous f) :
  continuous (λ p : X × X, dist (f p.1) (f p.2)) :=
...
theorem solutions_02_Metric_Spaces_5 {X Y : Type*} [metric_space X] [metric_space Y] {f : X → Y} (hf : continuous f) :
  continuous (λ p : X × X, dist (f p.1) (f p.2)) :=
begin
...
theorem solutions_02_Metric_Spaces_6 {X Y : Type*} [metric_space X] [metric_space Y] {f : X → Y} (hf : continuous f) :
  continuous (λ p : X × X, dist (f p.1) (f p.2)) :=
(hf.comp continuous_fst).dist (hf.comp continuous_snd)



theorem solutions_02_Metric_Spaces_7 {X Y : Type*} [metric_space X] [metric_space Y] {f : X → Y} (hf : continuous f) :
  continuous (λ p : X × X, dist (f p.1) (f p.2)) :=
hf.fst'.dist hf.snd'



theorem solutions_02_Metric_Spaces_8 {f : ℝ → X} (hf : continuous f) : continuous (λ x : ℝ, f (x^2 + x)) :=
sorry


theorem solutions_02_Metric_Spaces_9 {f : ℝ → X} (hf : continuous f) : continuous (λ x : ℝ, f (x^2 + x)) :=
hf.comp $ (continuous_pow 2).add continuous_id




theorem solutions_02_Metric_Spaces_10 {X Y : Type*} [metric_space X] [metric_space Y] (f : X → Y) (a : X) :
...
theorem solutions_02_Metric_Spaces_11 : metric.ball a r = {b | dist b a < r} := rfl

theorem solutions_02_Metric_Spaces_12 : metric.closed_ball a r = {b | dist b a ≤ r} := rfl



theorem solutions_02_Metric_Spaces_13 (hr : 0 < r) : a ∈ metric.ball a r := metric.mem_ball_self hr

theorem solutions_02_Metric_Spaces_14 (hr : 0 ≤ r) : a ∈ metric.closed_ball a r := metric.mem_closed_ball_self hr


theorem solutions_02_Metric_Spaces_15 (s : set X) : is_open s ↔ ∀ x ∈ s, ∃ ε > 0, metric.ball x ε ⊆ s :=
metric.is_open_iff



theorem solutions_02_Metric_Spaces_16 {s : set X} : is_closed s ↔ is_open sᶜ :=
is_open_compl_iff.symm

theorem solutions_02_Metric_Spaces_17 {s : set X} (hs : is_closed s) {u : ℕ → X} (hu : tendsto u at_top (𝓝 a))
  (hus : ∀ n, u n ∈ s) : a ∈ s :=
hs.mem_of_tendsto hu (eventually_of_forall hus)

theorem solutions_02_Metric_Spaces_18 {s : set X} : a ∈ closure s ↔ ∀ ε > 0, ∃ b ∈ s, a ∈ metric.ball b ε :=
metric.mem_closure_iff


theorem solutions_02_Metric_Spaces_19 {u : ℕ → X} (hu : tendsto u at_top (𝓝 a)) {s : set X} (hs : ∀ n, u n ∈ s) :
  a ∈ closure s :=
sorry


theorem solutions_02_Metric_Spaces_20 {u : ℕ → X} (hu : tendsto u at_top (𝓝 a)) {s : set X} (hs : ∀ n, u n ∈ s) :
  a ∈ closure s :=
begin
...
theorem solutions_02_Metric_Spaces_21 {x : X} {s : set X} : s ∈ 𝓝 x ↔ ∃ ε > 0, metric.ball x ε ⊆ s :=
metric.nhds_basis_ball.mem_iff

theorem solutions_02_Metric_Spaces_22 {x : X} {s : set X} : s ∈ 𝓝 x ↔ ∃ ε > 0, metric.closed_ball x ε ⊆ s :=
metric.nhds_basis_closed_ball.mem_iff




theorem solutions_02_Metric_Spaces_23 : is_compact (set.Icc 0 1 : set ℝ) :=
is_compact_Icc

theorem solutions_02_Metric_Spaces_24 {s : set X} (hs : is_compact s) {u : ℕ → X} (hu : ∀ n, u n ∈ s) :
  ∃ a ∈ s, ∃ φ : ℕ → ℕ, strict_mono φ ∧ tendsto (u ∘ φ) at_top (𝓝 a) :=
hs.tendsto_subseq hu

theorem solutions_02_Metric_Spaces_25 {s : set X} (hs : is_compact s) (hs' : s.nonempty)
  {f : X → ℝ} (hfs : continuous_on f s) :
  ∃ x ∈ s, ∀ y ∈ s, f x ≤ f y :=
hs.exists_forall_le hs' hfs

theorem solutions_02_Metric_Spaces_26 {s : set X} (hs : is_compact s) (hs' : s.nonempty)
  {f : X → ℝ} (hfs : continuous_on f s) :
  ∃ x ∈ s, ∀ y ∈ s, f y ≤ f x :=
hs.exists_forall_ge hs' hfs

theorem solutions_02_Metric_Spaces_27 {s : set X} (hs : is_compact s) : is_closed s :=
hs.is_closed



theorem solutions_02_Metric_Spaces_28 {X : Type*} [metric_space X] [compact_space X] : is_compact (univ : set X) :=
is_compact_univ
...
theorem solutions_02_Metric_Spaces_29 {X : Type*} [metric_space X] {Y : Type*} [metric_space Y] {f : X → Y} :
  uniform_continuous f ↔ ∀ ε > 0, ∃ δ > 0, ∀ {a b : X}, dist a b < δ → dist (f a) (f b) < ε :=
metric.uniform_continuous_iff



theorem solutions_02_Metric_Spaces_30 {X : Type*} [metric_space X] [compact_space X] {Y : Type*} [metric_space Y]
  {f : X → Y} (hf : continuous f) : uniform_continuous f :=
sorry


theorem solutions_02_Metric_Spaces_31 {X : Type*} [metric_space X] [compact_space X] {Y : Type*} [metric_space Y]
  {f : X → Y} (hf : continuous f) : uniform_continuous f :=
begin
...
theorem solutions_02_Metric_Spaces_32 (u : ℕ → X) : cauchy_seq u ↔ ∀ ε > 0, ∃ N : ℕ, ∀ m ≥ N,  ∀ n ≥ N, dist (u m) (u n) < ε :=
metric.cauchy_seq_iff

theorem solutions_02_Metric_Spaces_33 (u : ℕ → X) : cauchy_seq u ↔ ∀ ε > 0, ∃ N : ℕ, ∀ n ≥ N, dist (u n) (u N) < ε :=
metric.cauchy_seq_iff'


theorem solutions_02_Metric_Spaces_34 [complete_space X] (u : ℕ → X) (hu : cauchy_seq u) : ∃ x, tendsto u at_top (𝓝 x) :=
...
theorem solutions_02_Metric_Spaces_35 {u : ℕ → X} (hu : ∀ (n : ℕ), dist (u n) (u (n + 1)) ≤ (1 / 2) ^ n) :
  cauchy_seq u :=
begin
  rw metric.cauchy_seq_iff',
  intros ε ε_pos,
...
theorem solutions_02_Metric_Spaces_36 [complete_space X] (f : ℕ → set X) (ho : ∀ n, is_open (f n)) (hd : ∀ n, dense (f n)) : dense (⋂n, f n) :=
begin
  let B : ℕ → ℝ := λ n, (1/2)^n,
...
theorem solutions_02_Metric_Spaces_37 [complete_space X] (f : ℕ → set X) (ho : ∀ n, is_open (f n)) (hd : ∀ n, dense (f n)) : dense (⋂n, f n) :=
begin
  let B : ℕ → ℝ := λ n, (1/2)^n,
