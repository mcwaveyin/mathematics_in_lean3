...
theorem 01_Elementary_Integration_1 (a b : ℝ): ∫ x in a..b, x = (b ^ 2 - a ^ 2) / 2 :=
integral_id

theorem 01_Elementary_Integration_2 {a b : ℝ}  (h : (0:ℝ) ∉ [a, b]) : ∫ x in a..b, 1/x = real.log (b / a) :=
integral_one_div h

theorem 01_Elementary_Integration_3 (f : ℝ → ℝ) (hf : continuous f) (a b : ℝ) :
...
theorem 01_Elementary_Integration_4 {f : ℝ → ℝ} {a b : ℝ} {f' : ℝ → ℝ}
...
theorem 01_Elementary_Integration_5  (f : ℝ → ℝ) (g : ℝ → ℝ) :
  f ⋆ g = λ x, ∫ t, (f t) * (g (x - t)) :=
rfl
